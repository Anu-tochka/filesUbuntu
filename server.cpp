...
void server(string batch)  { 
    ...
    while(1){
        ...
        read(connection, batch, sizeof(batch));
            if (strncmp("end", batch, 3) == 0) {
                cout << "Client Exited." << endl;
                cout << "Server is Exiting..!" << endl;
                break;
            }
        cout << "Data received from client: " <<  batch << endl;
        bzero(batch, batch.length());
        cout << "Enter the batch you want to send to the client: " << endl;
        cin >> batch;
        ssize_t bytes = write(connection, batch, sizeof(batch));
